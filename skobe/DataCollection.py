...
# 		self.data = data
# 		self.genes = data.index.values
# 		self.annotations = None
# 		self.shape = data.shape
# 		self.columns = data.columns.values
# 	def setAnnotations(self, annotations):
# 		"""
# 		annotations: dataframe containing relevant annotation information
# 			index should be gene names
# 		"""
# 		self.annotations = annotations.reindex(self.genes).fillna('None')

# 	def filterFPKM(self, fpkm = 1):
# 		"""
...
# 	def plotHeatMap(self, option = 'heatmap', figsize = (5, 5)):
# 		if option == 'heatmap':
# 			fig, ax = plt.subplots(figsize= figsize)
# 			g = sns.heatmap(np.log2(self.data + 1))

# 		elif option == 'cluster':
# 			g = sns.clustermap(np.log2(self.data + 1), figsize = figsize)
# 		else:
# 			raise Exception('Invalid plot option')
# 		return g

# 	def join(self, df):
# 		return self.data.join(df)
